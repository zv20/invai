...
module.exports = (db, activityLogger) => {
  ...
  // Get batches for a product (all roles)
  ...
  // Get expiring batches (all roles)
  ...
  // Create batch (owner, manager, staff)
  router.post('/',
    authenticate,
  ...
  // Bulk create batches (owner, manager, staff)
  router.post('/bulk',
    authenticate,
    requirePermission('batches:create'),
    asyncHandler(async (req, res) => {
  ...
  // Adjust batch quantity (owner, manager, staff)
